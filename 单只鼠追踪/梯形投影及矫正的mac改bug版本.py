...
# ### MODIFIED ###: 分析范围设置 (使用秒)
START_SECONDS = 0.0  # 设置开始分析的秒数，0.0代表从头开始
...
if START_FRAME >= total_frames:
    print(
        f"错误：开始时间 ({START_SECONDS:.2f}s) 超出或等于视频总时长 ({video_duration_sec:.2f}s)。"
    )
    cap.release()
    exit()
...
if not ret:
    ...
    cap.release()
    exit()
print(f"视频将从 {START_SECONDS:.2f} 秒 (第 {START_FRAME} 帧) 开始进行设置和分析。")
