...
# --- 追踪参数 (请务必使用 hsv_debugger.py 工具获得最佳值) ---
...
OUTPUT_PLOT_PATH = os.path.join(OUTPUT_FOLDER, "trajectory_plot_pixels.png")
...
display_frame = None
start_frame_index = 0  # 新增：用于存储分析的起始帧
...
# --- 新增步骤 0: 选择分析的起始帧 ---
print("--- 步骤 0: 选择分析的起始帧 ---")
print("1. 拖动滑块，找到您希望开始分析的画面。")
print("2. 确认后，按 'c' 或 'Enter' 键。")
cv2.namedWindow("Step 0: Select Start Frame")
cv2.createTrackbar(
    "Frame", "Step 0: Select Start Frame", 0, total_frames - 1, start_frame_callback
)
start_frame_callback(0)  # 显示第一帧
while True:
    key = cv2.waitKey(20) & 0xFF
    if key in [ord("c"), 13]:
        start_frame_index = cv2.getTrackbarPos("Frame", "Step 0: Select Start Frame")
        break
cv2.destroyAllWindows()
print(f"分析将从第 {start_frame_index} 帧开始。")

# --- 步骤 1: 选择不规则ROI ---
display_frame = frame_for_selection.copy()  # 使用用户选定的帧作为ROI选择的背景
...
# --- 3. 初始化和主循环 (核心处理逻辑) ---
cap.set(cv2.CAP_PROP_POS_FRAMES, start_frame_index)  # 将视频指针移动到指定的起始帧
...
for frame_offset in tqdm(range(frames_to_process), desc="追踪进度"):
    ...
    if not ret:
        break
    time_sec = frame_offset / fps  # 时间从0开始计算
    ...
    for roi_id, polygon_points in enumerate(roi_polygons, 1):
        ...
        raw_trajectory_data.append([frame_num, time_sec, roi_id, abs_x, abs_y])
...
for frame, time_sec, roi_id, x_pixel, y_pixel in tqdm(
    raw_trajectory_data, desc="分析进度"
):
    distance_this_frame = 0.0
    prev_x, prev_y = last_coords[roi_id]
    if not (
        np.isnan(x_pixel) or np.isnan(y_pixel) or np.isnan(prev_x) or np.isnan(prev_y)
    ):
        distance_this_frame = np.sqrt((x_pixel - prev_x) ** 2 + (y_pixel - prev_y) ** 2)
...
for roi_id_to_summarize in range(1, len(roi_polygons) + 1):
    ...
    summary_filename = os.path.join(
        OUTPUT_FOLDER, f"summary_roi_{roi_id_to_summarize}_pixels.csv"
    )
