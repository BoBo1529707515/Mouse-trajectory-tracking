# 用于跟踪小鼠轨迹的方案,持续更新中  
## 多只鼠部分已更新，基于mmsegment的语义分割模型检测触摸
## 第八版：  更新至多框选、不规则、时间可移动、参数可调界面
## 遇到的问题请参见"设计思路与常见问题 (Troubleshooting).md"
## 地面倾斜请参考"梯形投影及修正部分使用指南.md"
## 步骤 1: 运行参数选取与代码开关(参考"参数选取指南.md")  


首先运行"参数选取.py"，并调整各个参数显示出小鼠为白色的情况并记录，随后我们将对应的参数输入“正式识别.py”部分中的参数。  
“正式识别.py”中  
SHOW_VIDEO_PREVIEW = True  
SAVE_OUTPUT_VIDEO = True  
True可以修改为False,这样能够选择是否观看视频或是否存储带标签视频。  
  
  
## 步骤 2: 选择分析的起始帧  

运行"正式识别,py"的界面(注意更改文件地址）: 程序会弹出一个名为 "Step 0: Select Start Frame" 的窗口，窗口下方有一个滑块。  
操作:  
...
## 理解输出文件
处理完成后，程序会在代码所在目录下创建一个与视频同名的文件夹。所有的输出结果都在这个文件夹里。  
